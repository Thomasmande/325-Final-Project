...
lm_numb_prenumb_interact <- lm(diffnumb ~ viewcat + prenumb + viewcat*prenumb, data = sesame)
...
q3_let <- lm(difflet ~ viewcat + site, data = sesame)
q3_numb <- lm(diffnumb ~ viewcat + site, data = sesame)
summary(q3_let)
summary(q3_numb)
```
...
```{r}
prune.numbers <- prune.tree(tree.nums, best = 4)
...
```{r}
ggplot(sesame, aes(x = difflet)) + geom_histogram() + theme_bw() + labs(title = "Distribution of Letter Test Score Improvement", x = "Score Improvement from Pre to Post Test", y = "Count")
ggplot(sesame, aes(x = diffnumb)) + geom_histogram() + theme_bw() + labs(title = "Distribution of Number Test Score Improvement", x = "Score Improvement from Pre to Post Test", y = "Count")
```
...
num_training_matrix <- model.matrix(formula(diffnumb ~ .), training)[,-1]
num_testing_matrix <- model.matrix(formula(diffnumb ~ .), testing)[,-1]
sesame_num <- training$diffnumb

#ridge.modNUM <- glmnet(num_training_matrix, sesame, alpha = 0, lambda = grid)
#ridge.num.pred <- predict(ridge.modNUM, num_testing_matrix)

#lasso.modNUM <- glmnet(num_training_matrix, training$diffnumb, alpha = 1, lambda = grid)

#mean((predict(lasso.modNUM, num_testing_matrix) - testing$diffnumb)^2)
```
```
